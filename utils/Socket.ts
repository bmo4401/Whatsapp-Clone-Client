...
const Socket = () => {
   ...
   useEffect(() => {
      if (socket && !socketEvent) {
         ...
         socket.on(
            SOCKET['INCOMING-VOICE-CALL'],
            (data: IncomingVoiceReceive) => {
         ...
         socket.on(
            SOCKET['INCOMING-VIDEO-CALL'],
            (data: IncomingVideoReceive) => {
               const { from, roomId, callType } = data;
               dispatch(
                  setIncomingVideoCall({
                     user: from,
         ...
         socket.on(SOCKET['VIDEO-CALL-REJECTED'], () => {
            dispatch(setEndCall());
         });
